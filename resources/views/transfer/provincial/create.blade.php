        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <span class="breadcrumb-item active">โอนเงิน</span>
                </div>
                <a href="#" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
            </div>
        </div>
                ...
                @foreach ($projectallocations as $key => $projectallocation)
                    @if (!Empty($allocationtransactions->where('subsidy_category_id',$projectallocation->subsidy_category_id)->first()))
                        <div class="card card-body border-top-info">
                            <form method="POST" action="{{route('transfer.provincial.createsave')}}" enctype="multipart/form-data">
                                @csrf
                                                ...
                                                @foreach ($allocationtransactions as $allocationtransaction)
                                                    @if ($allocationtransaction->subsidy_category_id == $projectallocation->subsidy_category_id)
                                                        <div class="form-group">
                                                            <label>{{$allocationtransaction->provincialsubareaeducation->name}} ({{number_format($allocationtransaction->budget, 2)}} บาท) <span class="text-danger">*</span> </label>
                                                            <input type="number" name="projecttransfer[{{$allocationtransaction->id}}]" value="" placeholder="" class="form-control" >
                                                        </div> 
                                                    @endif
                                                @endforeach
                                            </div>
