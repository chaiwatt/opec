                ...
                @endphp
                    @if (!Empty($check))
                        <div class="card card-body border-top-info">
                            <form method="POST" action="{{route('allocation.provincial.createsave')}}" enctype="multipart/form-data">
                                @csrf
                                ...
                                <div id="accordion-control-right-group{{$key+1}}" class="collapse" data-parent="#accordion-control-right">
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-12">
                                                @foreach ($provincialsubareaeducations as $provincialsubareaeducation)
                                                <div class="form-group">
                                                    <label>{{$provincialsubareaeducation->name}}<span class="text-danger">*</span></label>
                                                    <input type="number" name="projectallocation[{{$projectallocation->id}}][{{$provincialsubareaeducation->id}}]" value="" placeholder="" class="form-control" >
                                                </div>  
                                                @endforeach
                                            </div>
                                        </div>
