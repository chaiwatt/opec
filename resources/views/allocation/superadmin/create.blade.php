    ...
    <div class="page-header page-header-light">
        ...
        <div class="page-header-content header-elements-md-inline">
            ...
            <div class="header-elements d-none">
                <a href="{{route('project.create')}}" class="btn btn-labeled btn-labeled-right bg-primary">เพิ่มการจัดสรร <b><i class="icon-plus3"></i></b></a>
            </div>
        </div>
        ...
        <div class="row">
            ...
            <div class="card-group-control card-group-control-right " id="accordion-control-right">
                ...
                <div class="card card-body border-top-info">
                    <form method="POST" action="{{route('allocation.superadmin.createsave')}}" enctype="multipart/form-data">
                        @csrf
                        <div class="card-header">
                            <h6 class="card-title">
                                <a class="collapsed text-default" data-toggle="collapse" href="#accordion-control-right-group{{$key+1}}"> {{$projectallocation->subsidycategory->name}} ({{number_format($projectallocation->budget, 2)}} บาท)</a>
                            </h6>
                        </div>
                        <div id="accordion-control-right-group{{$key+1}}" class="collapse" data-parent="#accordion-control-right">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-12">
                                        @foreach ($provincialeducations as $provincialeducation)
                                        @php
                                            $check = $allocationtransactions->where('subsidy_category_id',$projectallocation->id)->where('provincial_education_id',$provincialeducation->id)->first();
                                            $value = 0;
                                            if(!Empty($check)){
                                                $value = $check->budget;
                                            }
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="number" name="projectallocation[{{$projectallocation->id}}][{{$provincialeducation->id}}]" value="{{$value}}" placeholder="" class="form-control" >
                                        </div>  
                                        @endforeach
                                    </div>
                                </div>
