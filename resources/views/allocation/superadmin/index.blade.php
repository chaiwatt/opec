    ...
    <div class="page-header page-header-light">
        ...
        <div class="page-header-content header-elements-md-inline">
            ...
            <div class="header-elements d-none">
                <a href="{{route('allocation.superadmin.create')}}" class="btn btn-labeled btn-labeled-right bg-primary">เพิ่มการจัดสรร <b><i class="icon-plus3"></i></b></a>
            </div>
        </div>
            ...
            <div class="card-group-control card-group-control-right " id="accordion-control-right">
                ...
                <div class="card card-body border-top-info">
                    <div class="card-header">
                        <h6 class="card-title">
                            <a class="collapsed text-default" data-toggle="collapse" href="#accordion-control-right-group{{$key+1}}">{{$subsidycategory->name}}</a>
                        </h6>
                    </div>
                                ...
                                <table class="table table-striped">
                                    ...
                                    <tbody>
                                        @foreach ($allocationtransactions as $key => $allocationtransaction)
                                            @if ($allocationtransaction->subsidy_category_id == $subsidycategory->id)
                                                <tr>   
                                                    <td> {{$key + 1}}</td>
                                                    <td> {{$allocationtransaction->createatthai}} </td> 
                                                    <td> {{$allocationtransaction->provincialeducation->name}} </td>                                                            
